...
//******** Entorn bàsic VS2019 MULTIFINESTRA amb OpenGL 3.3+, interfície MFC, llibreries GLM i primitives GLUT en codi font
...
// Llum #0 - (+Z)
//		if (lumin[0].encesa) {	
//	if (!ifix) glUniform4f(glGetUniformLocation(sh_programID, "LightSource[0].position"), 0.0, 0.0, 75.0, 1.0);//glLightfv(GL_LIGHT0, GL_POSITION, position);
//	else {	
...
//							}
	...
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[0].cutoff"), lumin[0].cutoff);
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[0].exponent"), lumin[0].exponent);
	//glEnable(GL_LIGHT0);
//	}
//	else glDisable(GL_LIGHT0);
	glUniform1i(glGetUniformLocation(sh_programID, "LightSource[0].sw_light"), lumin[0].encesa);

	position[0] = 200.0; position[1] = 0.0; position[2] = 0.0; position[3] = 1.0;
// Llum #1 (+X)
//		if (lumin[1].encesa) {	
//	if (!ifix) glUniform4f(glGetUniformLocation(sh_programID, "LightSource[1].position"), position[0], position[1], position[2], position[3]);//glLightfv(GL_LIGHT0, GL_POSITION, position);//glLightfv(GL_LIGHT1, GL_POSITION, position);
//		else {	
	// Conversió angles graus -> radians
...
//							}
	...
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[1].cutoff"), lumin[1].cutoff);
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[1].exponent"), lumin[1].exponent);
...
//	else glDisable(GL_LIGHT1);
	...
	position[0] = 0.0; position[1] = 200.0; position[2] = 0.0; position[3] = 1.0;
// Llum #2 (+Y)
//		if (lumin[2].encesa) {	
//	if (!ifix) glUniform4f(glGetUniformLocation(sh_programID, "LightSource[2].position"), position[0], position[1], position[2], position[3]);//glLightfv(GL_LIGHT2, GL_POSITION, position);
//		else {	
	// Conversió angles graus -> radians
...
//							}
	...
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[2].cutoff"), lumin[2].cutoff);
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[2].exponent"), lumin[2].exponent);
...
//	else glDisable(GL_LIGHT2);
	...
	position[0] = 200.0; position[1] = 200.0; position[2] = 200.0; position[3] = 1.0;
// Llum #3 (Z=Y=X)
//		if (lumin[3].encesa) {	
//	if (!ifix) glUniform4f(glGetUniformLocation(sh_programID, "LightSource[3].position"), position[0], position[1], position[2], position[3]); //glLightfv(GL_LIGHT3, GL_POSITION, position);
//		else {	
	// Conversió angles graus -> radians
...
//							}
	...
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[3].cutoff"), lumin[3].cutoff);
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[3].exponent"), lumin[3].exponent);
...
//	else glDisable(GL_LIGHT3);
	...
	position[0] = 0.0; position[1] = 0.0; position[2] = -200.0; position[3] = 1.0;
// Llum #4 (-Z)
//	if (lumin[4].encesa) {	
//	if (!ifix) glUniform4f(glGetUniformLocation(sh_programID, "LightSource[4].position"), position[0], position[1], position[2], position[3]); //glLightfv(GL_LIGHT4, GL_POSITION, position);
//		else {	
	// Conversió angles graus -> radians
...
//							}
	...
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[4].cutoff"), lumin[4].cutoff);
	glUniform1f(glGetUniformLocation(sh_programID, "LightSource[4].exponent"), lumin[4].exponent);
...
glm::mat4 Vista_Esferica(GLuint sh_programID,CEsfe3D opv,char VPol,bool pant,CPunt3D tr,CPunt3D trF,
				 ...
				 char iluminacio, bool llum_amb, LLUM* lumi, bool ifix, bool il2sides,
				 bool eix, CMask3D reixa, CPunt3D hreixa)
				 // antic bool eix, CMask3D reixa, CPunt3D hreixa, int camerax)
{    
	GLdouble cam[3],up[3];
	glm::mat4 MatriuVista(1.0);
	// Matrius Traslació
...
// Convertir angles Euler tg i tgF en Matrius
	if (TR) {	MatriuT = glm::eulerAngleX(tg.VRota.x);
				MatriuTG = MatriuTG * MatriuT;
			}
	MatriuT = glm::eulerAngleX(tgF.VRota.x);
	MatriuTG = MatriuTG * MatriuT;
	if (TR) {	MatriuT = glm::eulerAngleY(tg.VRota.y);
				MatriuTG = MatriuTG * MatriuT;
			}
	MatriuT = glm::eulerAngleY(tgF.VRota.y);
	MatriuTG = MatriuTG * MatriuT;
	if (TR) {	MatriuT = glm::eulerAngleZ(tg.VRota.z);
				MatriuTG = MatriuTG * MatriuT;
			}
	MatriuT = glm::eulerAngleX(tgF.VRota.z);
	MatriuTG = MatriuTG * MatriuT;
